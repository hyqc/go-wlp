...
func TestNewRWDirtyFilter(t *testing.T) {
	...
	assert.Equal(t, "你好啊，**个挨揍皮带哥三个挂**了啊李焕英Abc就好你好啊******个挨揍皮带哥三个**啊", filter.Replace(text, '*'), "错误")
}

func BenchmarkRWDirtyFilter_Replace2(b *testing.B) {
	f, err := os.Open("./dirtywords.csv")
	if err != nil {
		panic(err)
	}
	...
	filter := NewRWDirtyFilter(data)
	filter.Insert("张三")
	filter.InsertBatch([]string{"三哥哥", "四哥哥"})
...
func BenchmarkRWDirtyFilter_Replace(b *testing.B) {
	f, err := os.Open("./dirtywords.csv")
	if err != nil {
		panic(err)
	}
